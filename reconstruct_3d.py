...
def visualize_with_extrinsic(ply_path, agent_settings, extrinsic):
    ...
    vis.create_window(
        ...
        visible=True  # 如果你在本地有显示器可以设为 True
    )
...
def render_headless(ply_path, agent_settings, extrinsic, out_img="render.png"):
    # 1. load point cloud
    pcd = o3d.io.read_point_cloud(ply_path)

    # 2. offscreen size from your intrinsics
    W = agent_settings['intrinsic']['width']
    H = agent_settings['intrinsic']['height']
    renderer = rendering.OffscreenRenderer(W, H)  # uses OSMesa

    # 3. add geometry
    mat = rendering.MaterialRecord()
    renderer.scene.add_geometry("pcd", pcd, mat)

    # 4. build pinhole intrinsics
    intr = agent_settings['intrinsic']
    pinhole = o3d.camera.PinholeCameraIntrinsic(
        W, H,
        intr['fx'], intr['fy'],
    ...
    # 5. convert your world→camera to camera→world
    cam2world = np.linalg.inv(extrinsic).astype(np.float32)
    renderer.setup_camera(pinhole, cam2world)

    # 6. render and write out
    img = renderer.render_to_image()
    o3d.io.write_image(out_img, img)
    print(f"Headless render saved to {out_img}")
...
extrinsic = estimate_extrinsic_from_ray_depth(
        "data/output/view_001/view_001_rays.png",
        "data/output/view_001/view_001_depth.png",
        agent_settings
    )
# visualize_with_extrinsic("data/output/view_001/view_001.ply", agent_settings, extrinsic)
render_headless(
    "data/output/view_001/view_001.ply",
    agent_settings,
    extrinsic,
    out_img="headless.png"
)
